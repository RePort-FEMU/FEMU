...
class Emulator:
    def __init__(self, mode: RunningMode, inputPath: str, outputPath: str, brand: str = "auto", dbIP: str = "", dbPort: int = 5432):
        ...
        self.imagePath = outputPath + "/image"
    ...
    def extract(self):
        ...
        if not result["status"]:
            logging.error(f"Failed to extract kernel")
            return False
        
        self.kernelPath = str(result["kernelPath"])
        logging.debug(f"Kernel extracted to: {self.kernelPath}")

        # Now extract the root filesystem
        result = extract(self.inputPath, self.imagePath, kernel=True, sqlIP=self.dbIP, sqlPort=self.dbPort, brand=self.brand, quiet=True)[0]
        
        if not result["status"]:
            logging.error(f"Failed to extract root filesystem")
            if self.kernelPath:
                shutil.rmtree(self.kernelPath, ignore_errors=True)
            return False
        
        self.rootfsPath = str(result["rootfsPath"])
        
        logging.debug(f"Root filesystem extracted to: {self.rootfsPath}")
        
        if not self.kernelPath or not self.rootfsPath:
            logging.error("Extraction failed: Kernel or root filesystem path is empty.")
            return False
        
        logging.info(f"Extraction completed successfully. Kernel: {self.kernelPath}, RootFS: {self.rootfsPath}")
    ...
    def inferArchitecture(self):
        ...
        self.architecture, self.endianess = checkArch(self.inputPath, self.iid)
    ...
    def inferKernelVersion(self):
        ...
        for string in strings(self.kernelPath, minLength=4):
            if "Linux version" in string:
                self.kernelVersion = string.split("Linux version ")[1].split(" ")[0]
                self.kernelVersionString = string
                logging.debug(f"Found kernel version: {self.kernelVersion}")
            elif "init=" in string:
                self.kernelInit = string.split("init=")[1].split(" ")[0]
    ...
    def collectInfo(self):
        ...
        if not self.iid or not self.kernelPath or not self.rootfsPath:
